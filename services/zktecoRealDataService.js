...
try {
  ...
  try {
    ...
    console.log('⚠️ node-zklib import also failed for ZKTecoService:', nodeZklibError.message);
  }
}
...
try {
  ...
  console.log('⚠️ js-zklib import failed for ZKTecoService:', jsImportError.message);
}
...
class ZKTecoService {
  ...
  async connect() {
    try {
      ...
      if (!ZKLib) {
        throw new Error('ZKTeco libraries not available. Please ensure zklib is properly installed.');
      }
      ...
      try {
        ...
        this.zkInstance = new ZKLib({
          inport: 4000, // Local UDP port for zklib (different from device port)
          ip: this.ip,
          port: parseInt(this.port), // Device port (4370)
          timeout: 10000
        });
        console.log(`✅ Options object constructor success (inport style)`);
      } catch (optionsError) {
        console.log(`⚠️ Options constructor (inport) failed: ${optionsError.message}`);

        try {
          // Pattern 2: Alternative options format
          this.zkInstance = new ZKLib({
            ip: this.ip,
            inport: 4000, // Local UDP port for zklib (different from device port)
            port: parseInt(this.port), // Device port (4370)
          ...
          console.log(`✅ Alternative options constructor success`);
        } catch (altOptionsError) {
          console.log(`⚠️ Alternative options constructor failed: ${altOptionsError.message}`);

          try {
            // Pattern 3: Simple constructor
            ...
            try {
              // Pattern 4: With timeout parameter
      ...
      const connectPromise = new Promise((resolve, reject) => {
        try {
          ...
          try {
            ...
            reject(new Error(`Connection failed: ${syncError.message} | ${noCallbackError.message}`));
          }
        }
      });
  ...
  async getUsers() {
    ...
    try {
      ...
      if (typeof this.zkInstance.getUser !== 'function') {
        throw new Error('getUser method not available - device may not support user management via SDK');
      }
      ...
      if (typeof this.zkInstance.disableDevice === 'function') {
        await new Promise((resolve, reject) => {
          this.zkInstance.disableDevice((err) => {
            if (err) {
              console.log('⚠️ Could not disable device, continuing anyway:', err.message || err);
            }
      ...
      console.log(`✅ Retrieved users data:`, typeof usersData, Array.isArray(usersData) ? usersData.length : 'unknown');
      ...
      if (Array.isArray(usersData)) {
        ...
        console.log('⚠️ Unexpected users response format, returning empty array');
        userArray = [];
      }
      ...
      const formattedUsers = userArray.map(user => ({
        ...
        cardno: user.cardno || user.cardNumber || user.employeeId || user.uid || 'NO_CARD',
        role: user.role || 0,
      ...
      console.log(`✅ Processed ${formattedUsers.length} users from ZKTeco device`);
  ...
  async getAttendanceLogs(startDate = null) {
    ...
    try {
      ...
      if (typeof this.zkInstance.getAttendance !== 'function') {
        throw new Error('getAttendance method not available - device may not support attendance log retrieval via SDK');
      }
      ...
      const getAttendancePromise = new Promise((resolve, reject) => {
        ...
        this.zkInstance.getAttendance((err, attendanceData) => {
          clearTimeout(timeout);
          if (err) {
            reject(new Error(`Failed to get attendance logs: ${err.message || err}`));
          } else {
      ...
      console.log(`✅ Retrieved attendance data:`, typeof attendanceData, Array.isArray(attendanceData) ? attendanceData.length : 'unknown');
      ...
      if (Array.isArray(attendanceData)) {
        ...
        logsArray = attendanceData.data || attendanceData.logs || attendanceData.records || attendanceData.result || [attendanceData];
      } else {
        console.log('⚠️ Unexpected attendance response format, returning empty array');
        logsArray = [];
      }
      ...
      console.log(`✅ Processed ${formattedLogs.length} attendance logs from ZKTeco device`);
    ...
    try {
      ...
      const getAttendancePromise = new Promise((resolve, reject) => {
        ...
        this.zkInstance.getAttendance((err, attendanceData) => {
          clearTimeout(timeout);
          if (err) {
            reject(new Error(`Failed to get attendance: ${err.message || err}`));
          } else {
            resolve(attendanceData || []);
          }
        });
      ...
      console.log(`✅ Retrieved attendance data: ${Array.isArray(attendanceData) ? attendanceData.length : typeof attendanceData} records`);
  ...
  getAvailableMethods() {
    ...
    const methodsToCheck = [
      'getUsers', 'getAttendances', 'getInfo', 'getTime',
      'getLogs', 'getRecords', 'getRealTimeData'
    ];

    return methodsToCheck.filter(method =>
      typeof this.zkInstance[method] === 'function'
    );
  }
  ...
  initialize(zkInstances, machineConnections) {
    try {
      ...
      console.log(`✅ zktecoRealDataService initialized with ${Object.keys(zkInstances || {}).length} ZK instances and ${Object.keys(machineConnections || {}).length} machine connections`);
      ...
      console.error('❌ Failed to initialize zktecoRealDataService:', error.message);
      throw error;
    }
  }
  ...
  static async getEmployeeAttendanceReal(ip, employeeId, startDate, endDate, companyId, forceSync = false) {
    try {
      console.log(`🔧 Getting employee attendance: ${employeeId} from ${ip} (${startDate} to ${endDate})`);
      ...
      const employeeLogs = attendanceLogs.filter(log =>
        log.uid === employeeId || log.userId === employeeId
      );
    ...
    try {
      ...
      if (zkInstance && typeof zkInstance.getInfo === 'function') {
        try {
          ...
          diagnostics.capabilities.sdkLibraryIssues.push(`getInfo failed: ${error.message}`);
        }
      }
  ...
  static async fetchRealAttendanceLogsBatched(ip, startDate, endDate, companyId, batchSizeDays = 7) {
    try {
