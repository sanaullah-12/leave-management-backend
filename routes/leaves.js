  ...
  try {
    ...
    if (company.leavePolicy?.preventOverlappingLeaves) {
      ...
      if (companyOverlappingLeaves.length > 0) {
        const conflictingEmployees = companyOverlappingLeaves
          .map(
    ...
    if (
      company.leavePolicy?.maxConcurrentLeaves &&
      company.leavePolicy.maxConcurrentLeaves > 0
    ) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // Send notifications to employee (non-blocking)
      let emailNotificationFailed = false;
      let inAppNotificationFailed = false;

      try {
        // Try to send email notification to employee
        await sendLeaveStatusNotification(leave.employee, leave, req.user);
      } catch (emailError) {
        console.error(
          `Failed to send email notification to employee ${leave.employee.email}:`,
          emailError.message
        );
      ...
      try {
        ...
        console.error(
          `Failed to send in-app notification to employee ${leave.employee.name}:`,
          notificationError.message
        );
        inAppNotificationFailed = true;
      }

      // Prepare response message
      let message = `Leave request ${status} successfully`;
      let warnings = [];

      if (emailNotificationFailed) {
        warnings.push("Email notification to employee may have failed");
      }
      if (inAppNotificationFailed) {
        warnings.push("In-app notification may have failed");
      }

      if (warnings.length > 0) {
        message +=
          ". " + warnings.join(", ") + ", but the status has been updated.";
      }
...
router.get("/balance/:employeeId", authenticateToken, async (req, res) => {
  try {
    ...
    const balance = {
      annual: {
        total: correctedPolicy.annualLeave,
        used: usedLeaves.annual,
        remaining: correctedPolicy.annualLeave - usedLeaves.annual,
      },
      sick: {
        total: correctedPolicy.sickLeave,
        used: usedLeaves.sick,
        remaining: correctedPolicy.sickLeave - usedLeaves.sick,
      },
      casual: {
        total: correctedPolicy.casualLeave,
        used: usedLeaves.casual,
        remaining: correctedPolicy.casualLeave - usedLeaves.casual,
      },
      maternity: {
        total: correctedPolicy.maternityLeave,
        used: usedLeaves.maternity,
        remaining: correctedPolicy.maternityLeave - usedLeaves.maternity,
      },
      paternity: {
        total: correctedPolicy.paternityLeave,
        used: usedLeaves.paternity,
        remaining: correctedPolicy.paternityLeave - usedLeaves.paternity,
      },
    };
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (
        policy.maxConcurrentLeaves !== null &&
        policy.maxConcurrentLeaves !== undefined
      ) {
...
router.post(
  "/policy/max-concurrent/:maxCount",
  authenticateToken,
  authorizeRoles("admin"),
  async (req, res) => {
    try {
