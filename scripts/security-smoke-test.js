...
(async () => {
  ...
  const bad1 = await call("POST", "/auth/login", { body: { email: `emp1-${uniq}@example.com`, password: "wrong-password1" } });
  const bad2 = await call("POST", "/auth/login", { body: { email: `nobody-${uniq}@example.com`, password: "wrong-password1" } });
  ...
  let locked = false;
  for (let i = 0; i < 7; i++) {
    const r = await call("POST", "/auth/login", { body: { email: `emp2-${uniq}@example.com`, password: "Wrong-pass-123" } });
